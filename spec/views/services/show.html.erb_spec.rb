...
RSpec.describe "services/show" do
  it "displays @cart, @categories, @reviews, @contact the services" do
    assign(:services, [
      ...
      @service = create(:service, title: 'Услуга', description: 'Описание')
    ])
    assign(:services, [
      create(:service, title: 'Услуга еще'),
      create(:service, title: 'Услуга еще вторая')
    ])
    ...
    expect(rendered).to match /Услуга/
    expect(rendered).to match /Описание/
    expect(rendered).to match /Услуга еще/
    expect(rendered).to match /Услуга еще вторая/
  end
end